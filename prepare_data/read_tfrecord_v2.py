...
def parse_tfrecord(example_proto, image_size):
    ...
    label = tf.cast(example['image/label'], tf.float32)
    roi = example['image/roi']
    landmark = example['image/landmark']
    return image, label, roi, landmark

def read_single_tfrecord(tfrecord_file, batch_size, net):
    image_size = 12 if net == 'PNet' else (24 if net == 'RNet' else 48)
    dataset = tf.data.TFRecordDataset([tfrecord_file])
    dataset = dataset.map(lambda x: parse_tfrecord(x, image_size))
