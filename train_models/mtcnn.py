...
def create_pnet(width, height):
    input = keras.layers.Input(shape=[width, height, 3])
    x = keras.layers.Conv2D(10, (3, 3), strides=1, padding='valid', 
                            kernel_regularizer=keras.regularizers.l2(0.0005),name='conv1')(input)
    x = keras.layers.PReLU(shared_axes=[1,2],name='PReLU1')(x)
    x = keras.layers.MaxPool2D(pool_size=2)(x)
    
    x = keras.layers.Conv2D(16, (3, 3), strides=1, padding='valid',
                             kernel_regularizer=keras.regularizers.l2(0.0005), name='conv2')(x)
    x = keras.layers.PReLU(shared_axes=[1,2],name='PReLU2')(x)

    x = keras.layers.Conv2D(32, (3, 3), strides=1, padding='valid', 
                            kernel_regularizer=keras.regularizers.l2(0.0005), name='conv3')(x)
    ...
    landmark_pred = keras.layers.Conv2D(10, (1, 1), name='conv4-3')(x) 

    model = keras.models.Model([input], [classifier, bbox_regress, landmark_pred])
...
def create_rnet(weights_file):
    ...
    classifier = keras.layers.Dense(2, activation='softmax', name='dense2-1')(x)
    bbox_regress = keras.layers.Dense(4, name='dense2-2')(x)
    landmark_pred = keras.layers.Dense(10, name='dense2-3')(x)
...
def create_onet(weights_file):
    ...
    x = keras.layers.Permute((3,2,1))(x)
    x = keras.layers.Flatten()(x)
    x = keras.layers.Dense(256, name='dense1') (x)
    x = keras.layers.PReLU(name='prelu5')(x)

    classifier = keras.layers.Dense(2, activation='softmax',name='dense2-1')(x)
    bbox_regress = keras.layers.Dense(4,name='dense2-2')(x)
    landmark_pred = keras.layers.Dense(10,name='dense2-3')(x)
    model = keras.models.Model([input], [classifier, bbox_regress, landmark_pred])#, landmark_regress])
    # model.load_weights(weights_file, by_name=True)
    return model